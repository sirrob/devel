...
class news_events extends mb {
    ...
    function get_baner($id) {
        ...
        $query = "select * from " . dn('news') . "
                  where page=2 and alias='".$id."' and (active='y' or (active_start<'" . date("Y-n-d") . "' and 
                        active_stop<'" . date('Y-n-d') . "')) and langid=" . $_GLOBAL['langid'] . ";";
        //echo $query;
        $re = $db->query($query);
        $item = $db->fetch($re);
        ...
        (file_exists('./files/news/' . $item['file1']) && is_file('./files/news/' . $item['file1'])) ) {
            ...
            {
                $img = '<object width="788" height="374" bgcolor="black" type="application/x-shockwave-flash" data="' . $_GLOBAL['page_url'] . 'files/player.swf" style="background-color: black">
				<param name="movie" value="' . $_GLOBAL['page_url'] . 'files/player.swf" />
                                <param name="wmode" value="transparent">
                                <param name="bgcolor" value="black">
				<param name="flashvars" value="controlbar=over&amp;autostart=false&amp;image=' . $_GLOBAL['page_url'] . 'files/news/' . str_replace('mp4','jpg',$item['file0']) .'&amp;file=' . $_GLOBAL['page_url'] . 'files/news/' . $item['file0'] . '" />
                                <embed src="' . $_GLOBAL['page_url'] . 'files/player.swf" FlashVars="controlbar=over&amp;autostart=false&amp;image=' . $_GLOBAL['page_url'] . 'files/news/' . str_replace('mp4','jpg',$item['file0']) .'&amp;file=' . $_GLOBAL['page_url'] . 'files/news/' . $item['file0'] . '"  menu="false" scale="scale" wmode="transparent" quality="high" width="788" height="374" bgcolor="black" name="flash_id" align="middle" allowscriptaccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
                            </object>';
            }
...
//            }
            ...
            if($roz[count($roz)-1]=='mp4')
            {
				...
				<param name="movie" value="' . $_GLOBAL['page_url'] . 'files/player.swf" />
                                <param name="wmode" value="transparent">
				<param name="flashvars" value="autostart=false&amp;controlbar=over&amp;image=' . $_GLOBAL['page_url'] . 'files/news/' . str_replace('mp4','jpg',$item['file1']) .'&amp;file=' . $_GLOBAL['page_url'] . 'files/news/' . $item['file1'] . '" />
                                <embed src="' . $_GLOBAL['page_url'] . 'files/player.swf" FlashVars="autostart=false&amp;controlbar=over&amp;image=' . $_GLOBAL['page_url'] . 'files/news/' . str_replace('mp4','jpg',$item['file1']) .'&amp;file=' . $_GLOBAL['page_url'] . 'files/news/' . $item['file1'] . '"  menu="false" scale="scale" wmode="transparent" quality="high" width="194" height="374" name="flash_id" align="middle" allowscriptaccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
                            </object>';
            }
...
//            }
            ...
            if (isset($item['product_index'.$i]) AND !empty($item['product_index'.$i])) {
                ...
                $out .= '<div class="news-product-block">
                            <a href="'.$product_link.'" class="with-tooltip">
                                ...
                                <div class="product-manufacture">'.$product_producer.'</div>
    ...
    function get_strona() {
        ...
        if(count($sURL)==2) {
            ...
            $query = "select * from " . dn('news') . "
                    where page=2 and (active='y' and ((active_start<'" . date("Y-n-d") . "' or active_start is NULL) and (active_stop>='" . date('Y-n-d') . "' or active_stop is NULL))) and
            ...
            if(file_exists('./files/news/' . $item['file2']) && is_file('./files/news/' . $item['file2'])) {
                ...
                if($roz[count($roz)-1]=='jpg')
                {
                    if(!empty($item['link2'])) $img2 = '<a href="' . $item['link2'] . '"><img src="' . $_GLOBAL['page_url'] . 'files/news/' . $item['file2'] . '" alt=""></a>';
                    else $img2 = '<img src="' . $_GLOBAL['page_url'] . 'files/news/' . $item['file2'] . '" alt="">';
                } else
                if($roz[count($roz)-1]=='swf')
                {
				...
				<param name="movie" value="' . $_GLOBAL['page_url'] . 'files/player.swf" />
                                <param name="wmode" value="transparent">
				<param name="flashvars" value="autostart=false&amp;controlbar=over&amp;image=' . $_GLOBAL['page_url'] . 'files/news/' . str_replace('mp4','jpg',$item['file2']) .'&amp;file=../files/news/' . $item['file2'] . '" />
                                <embed src="' . $_GLOBAL['page_url'] . 'files/player.swf" FlashVars="autostart=false&amp;controlbar=over&amp;image=' . $_GLOBAL['page_url'] . 'files/news/' . str_replace('mp4','jpg',$item['file2']) .'&amp;file=../files/news/' . $item['file2'] . '"  menu="false" scale="scale" wmode="transparent" quality="high" width="194" height="374" name="flash_id" align="middle" allowscriptaccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
                            </object>';
                } 
            }
    		...
    		// SMALL BANNERS
        $out .= '<div id="small-banners" style="float: left">';
                
        //pobieranie z bazy newsów        
        $query = "select * from " . dn('news') . "
                  where page=2 and alias<>'".$news_id."' and (active='y' and ((active_start<'" . date("Y-n-d") . "' or active_start is NULL) and 
                        (active_stop>='" . date('Y-n-d') . "' or active_stop is NULL))) and
                        langid=" . $_GLOBAL['langid'] . "
        ...
        {//<a href="news_events,' . $item['id'] . '.htm">Czytaj więcej...</a>
            $out .= '<div class="box">
                        <div class="ic_container">
                            ...
                            <div class="ic_caption">
                                <p>' . $item['abstract'] . '</p>
				<a href="' . $_GLOBAL['page_url'] . $_GLOBAL['lang'] . '/news_events/' . $item['alias'] . '/">' . $L['{T_CZYTAJ_WIECEJ}'] . '</a>
                            </div>
			</div>
                    </div><!-- box -->	';
        }
