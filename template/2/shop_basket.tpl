...
<tr class="r{MOD}">
	...
	<td>
		...
		{NAZWA} (roz. {ATRYBUT})<br/>
		{INFO}<br/>
		{T_CENA}: {CENA1} zł
		{CENA2}
		<input type="hidden" id="cena_{ID}_{AT_ID}" value="{CENAFORM}">
	</td>
	<td><input type="text" name="ko_ile_{ID}_{AT_ID}" id="ko_ile_{ID}_{AT_ID}" class="ilosc{ERROR}" value="{ILE}" maxlength="3" onblur="m_przelicz()" /></td>
	<td id="wartosc_{ID}_{AT_ID}">{WARTOSC} zł</td>
	<td><input type="checkbox" name="ko_usun_{ID}_{AT_ID}"></td>
</tr>
<!--/{IT_ITEM}-->
...
<select name="do_id" id="do_id" onchange="m_przelicz()" class="txtl" style="width:250px;">
